...
func main() {
  command := flag.String('-A', '--Append', help = 'Append and right justify string with Number and Separator')
}
...
def main():
    ...
    if args.Number == None:
        if args.Append != None: args.Number = shutil.get_terminal_size()[0]
        elif args.convert != None: args.Number = shutil.get_terminal_size()[0]
        elif args.Every != None: args.Number = 1
        elif args.enumerate: args.Number = 1

    if args.Separator == None:
        if args.Append: args.Separator = ' '
        elif args.append: args.Separator = ''
        elif args.convert: args.Separator = ' '
        elif args.enumerate: args.Separator = '.'
    ...
    for count, line in enumerate(fileinput.input(files = args.input_files), start = 1):
        if not filter_line(args, line, count): continue
        num += 1
        o = process_line(args, line, num)
        print(o, end='')

def filter_line(args, line, count):
    if args.number and (count > args.number): return False
    if args.Every:
        if 'even' in args.Every and count % 2 != 0: return False
        if 'number' in args.Every and count % args.Number != 0: return False
        if 'odd' in args.Every and count % 2 != 1: return False
    if args.find and (args.find not in line): return False
    if args.Find and (args.Find.lower() not in line.lower()): return False
    if args.grep and not re.search(args.grep, line): return False
...
def process_line(args, line, num):
    ...
    if args.Append: l = f'{l}{args.Append.rjust(args.Number - len(l), args.Separator)}'
    if args.append: l = f'{l}{args.Separator}{args.append}'
    if args.convert:
